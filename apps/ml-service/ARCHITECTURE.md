...
The prompt is sent to a local `qwen3:8b` instance over `localhost:11434` with
...
Rather than describing what math *is*, the algorithm looks for characters that
...
6. Operator replacements: `×` → `\times`, `÷` → `\div`, ` / ` → `\div`, `%` → `\%`
7. Remaining bare digit sequences wrapped in `\text{}` (required by LaTeXSwiftUI)
